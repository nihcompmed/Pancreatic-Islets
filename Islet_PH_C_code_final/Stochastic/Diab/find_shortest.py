...
def single_job(job):
    ...
    #for fprefix in these_islets:
    
    islet_dirr = dirr + '/' + fprefix
    
    print('\nProcessing', fprefix, typ)
    ...
    for max_pert in max_perts:
        ...
        for pert in range(n_pert):
            ...
            for perm in range(n_perm):
                ...
                if typ == 'ad':
                    ...
                    undead_file = this_perm_dirr + '/bundead_PH.csv'
        ...
        this_info[max_pert] = mantle_info
...
for fprefix in far_ad_mantles:
    jobs.append([fprefix, 'ad'])

for fprefix in far_b_mantles:
    jobs.append([fprefix, 'b'])
...
for idx, job in enumerate(jobs):
    fprefix, typ = job
    all_info[fprefix] = dict()
    all_info[fprefix][typ] = results[idx]


pickle.dump(all_info, open('all_short_mantle_info.p', 'wb'))
